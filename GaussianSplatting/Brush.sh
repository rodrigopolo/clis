...
for MODEL_DIR in "${SCENE_DIR}/sparse"/*/; do
  if [[ -f "${MODEL_DIR}/images.bin" ]]; then
    COUNT=$(colmap model_analyzer --path "${MODEL_DIR}" 2>&1 | grep "Registered images" | awk '{print $NF}')
    log "Model ${MODEL_DIR}: ${COUNT} registered images."
...
(cd "${SCENE_DIR}" && zip -r0 "sparse_backup.zip" "sparse/")
wait
